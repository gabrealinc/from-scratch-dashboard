...
import { buildSnapshot, sheetPlan, type RawPaymentReport, type RawReport } from "@/lib/reporting";
...
export async function POST(request: Request) {
  ...
  try {
    const body = JSON.parse(text) as {phase:"prepare"|"commit";reports:RawReport[];paymentReports?:RawPaymentReport[];sheetVerified?:boolean};
    const snapshot=await convertToUsd(buildSnapshot(body.reports,body.paymentReports??[]));
    ...
    return Response.json({ok:true,copies:snapshot.totals.copies,royalties:snapshot.totals.royalties,payouts:snapshot.payouts?.totalUsd??0,syncedAt:snapshot.syncedAt});
  } catch (error) {
